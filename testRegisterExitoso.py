...
import time
from credenciales import validName, email, password
...
#ingresar datos
try:
    nameBox.send_keys(validName)
    emailBoxRegister.send_keys(email)
    ...
    actions.move_to_element(passBoxRegister).click().send_keys(password).perform()
    actions.move_to_element(confirmPassBoxRegister).click().send_keys(password).perform()
    time.sleep(2)
